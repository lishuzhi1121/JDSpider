...
class MySQLPipeline(object):
	...
	def spider_closed(self, spider):
		""" Cleanup function, called after crawing has finished to close open objects. Close ConnectionPool. """
		self.dbpool.close()
	...
	def _insert_record(self, tx, item):
        ...
        sql = "INSERT INTO jd_goods VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (ID, name, comment_num, shop_name, link, commentVersion, score1count, score2count, score3count, score4count, score5count, price)
        tx.execute(sql)
        print "yes"
...
class CommentPipeline(object):
	...
	def __init__(self, stats):
		# super(CommentPipeline, self).__init__()
		self.dbpool = adbapi.ConnectionPool('MySQLdb',
                                            host=SETTINGS['DB_HOST'],
                                            user=SETTINGS['DB_USER'],
                                            passwd=SETTINGS['DB_PASSWD'],
    ...
    def _insert_record(self, tx, item):
        ...
        sql = "INSERT INTO jd_comment VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s')" % (user_name, user_ID, userProvince, content, good_ID, good_name, date, replyCount, score, status, title, userRegisterTime, productColor, productSize, userLevelName, isMobile, days, tags)
        tx.execute(sql)
        print "yes"
