...
# Spider JD product's comments

from scrapy.spiders import Spider
from JDSpider.items import goodsItem, commentItem
...
class comment_spider(Spider):
    ...
    for i in range(len(good_id)):	# 一件商品一件商品的抽取
    	...
    	if comment_total % 10 == 0:	# 算出评论页数,一页10条评论
    		page = comment_total/10
    	else:
    		page = comment_total/10 + 1

    	for k in range(0, page):
    		url = 'http://sclub.jd.com/productpage/p-' + str(good_num) + '-s-0-t-3-p-' + str(k) + '.html?callback=fetchJSON_comment98vv' + str(comment_V[i])
